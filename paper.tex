...
\begin{enumerate}
  ...
  \item ANNOds (\cref{sec:domain}) is a domain ontology for describing domain-specific entities to be used for annotating 
...
(i.e., the ANNOds classes are subclasses of the ANNOdc classes).
  ...
  \item ANNOdc (\cref{sec:core}, \cref{fig:core}) is a core ontology to provide the core entities of a domain, such as, inter alia, 
...
the category \enquote{Phenotype} to model the rules for determining human phenotypes. The ANNOdc is founded by \emph{General Formal Ontology} (GFO)~\citep{gfo}.
...
In our case, the ANNOdc plays the role of a task ontology and serves as the conceptual model of the software. 
...
of the task ontology (ANNOdc), whereas the entities of the corresponding domain ontology (ANNOds) are processed dynamically. 
The \aw{} uses the ANNOdc as an interface to access the ANNOds entities, e.g. by listing all subclasses of a specific ANNOdc class, such as \aurl{anno}{Bone}, as described in \cref{sec:aw}.
